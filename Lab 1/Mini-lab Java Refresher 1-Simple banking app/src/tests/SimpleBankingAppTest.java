...
public class SimpleBankingAppTest {
	...
	public static void testWithdrawals() {
		// 1-Setup phase
		double balanceBefore = SimpleBankingApp.getBalance("5495-1234");
		double withdrawalsAmount = 50;

		// 2-Exercise phase
		SimpleBankingApp.addTransaction("5495-1234", -withdrawalsAmount);
		double balanceAfter = SimpleBankingApp.getBalance("5495-1234");

		// 3-verify
        assert balanceBefore - withdrawalsAmount == balanceAfter;
		if (balanceBefore - withdrawalsAmount == balanceAfter)
			System.out.println(TestUtils.TEXT_COLOR_GREEN + "testWithdrawals: TC1 passed"+ TestUtils.TEXT_COLOR_RESET);
		else {
			System.out.println(TestUtils.TEXT_COLOR_RED + "testWithdrawals: TC1 FAILED XXX: balanceBefore + withdrawalsAmount != balanceAfter");
			System.out.format("testWithdrawals: withdrawalsAmount = %.2f ; withdrawalsAmount = %.2f ; balanceAfter = %.2f %s\n",
					balanceBefore , withdrawalsAmount , balanceAfter, TestUtils.TEXT_COLOR_RESET);
		}
